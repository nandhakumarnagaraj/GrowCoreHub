...
public class AssessmentTestController {
	...
	/**
	 * Create a sample Java assessment for testing
	 */
	...
	public ResponseEntity<?> createSampleJavaAssessment(@PathVariable Long projectId) {
		try {
			...
			response.put("questionsPreview", questionsJson);
			response.put("totalPoints", AssessmentUtils.calculateTotalPoints(questionsJson));
	...
	/**
	 * Submit sample answers to an assessment for testing
	 */
	...
	public ResponseEntity<?> submitSampleAnswers(@PathVariable Long assessmentId, @RequestParam Long userId,
		...
		try {
			...
			response.put("submittedAnswers", answers);
	...
	public ResponseEntity<?> validateQuestionsJson(@RequestBody Map<String, String> request) {
		try {
			...
			if (isValid) {
				response.put("questionIds", AssessmentUtils.getQuestionIds(questionsJson));
				response.put("totalPoints", AssessmentUtils.calculateTotalPoints(questionsJson));
			} else {
				response.put("message", "Invalid assessment JSON structure");
			}
	...
	public ResponseEntity<?> validateAnswersJson(@RequestBody Map<String, String> request) {
		try {
			...
			if (!isValid) {
				response.put("message", "Invalid answers JSON structure");
			}
	...
	/**
	 * Create assessment using the builder pattern
	 */
